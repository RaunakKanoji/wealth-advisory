...
import { getRemoteCard, getRemoteCardTransactions } from "@/lib/api/cards";
import { apiCardToCardRecord, apiCardTransactionToCardTransaction } from "@/lib/api/view-models";
...
export function CardTransactionDetailsScreen() {
  ...
  const customerId = isRemoteDataEnabled ? user?.id : user?.id ?? DEMO_CUSTOMER_A;
  ...
  useEffect(() => {
    ...
    if (isRemoteDataEnabled && (!isUserLoaded || !isFinancialAuthReady(isAuthLoaded, isSignedIn))) return () => { active = false; };
    const request = isRemoteDataEnabled
      ? Promise.all([getRemoteCard(cardId, { getToken }), getRemoteCardTransactions(cardId, { getToken })]).then(([remoteCard, remoteTransactions]) => [
          apiCardToCardRecord(remoteCard),
          remoteTransactions.items.map(apiCardTransactionToCardTransaction).find((item) => item.id === transactionId),
        ] as const)
      : Promise.all([
          getCard(cardId, { customerId }),
          getCardTransaction(cardId, transactionId, { customerId }),
        ]);
    void request.then(([nextCard, nextTransaction]) => {
      if (!active) return;
    ...
    return () => { active = false; };
  }, [cardId, customerId, getToken, isAuthLoaded, isSignedIn, isUserLoaded, transactionId]);
